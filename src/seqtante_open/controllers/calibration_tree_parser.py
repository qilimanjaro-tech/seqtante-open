...
class CalibrationParser:
    ...
    @staticmethod
    def _make_experiment_path(base_folder: str, target: str, name: str) -> str:
    ...
    def _check_topology(self, targets):
        if "all_q" in targets:
            if (
                self.platform.analog_compilation_settings is None
                or self.platform.analog_compilation_settings.qubits is None
            ):
                raise ValueError("all_q is not a valid target. Add qubits inside runcard's analog.")
            qubit_targets = [f"q{qubit}" for qubit in self.platform.analog_compilation_settings.qubits]
            targets = [t for t in targets if t != "all_q"] + qubit_targets
        if "all_c" in targets:
            if (
                self.platform.analog_compilation_settings is None
                or self.platform.analog_compilation_settings.topology is None
            ):
                raise ValueError("all_c is not a valid target. Add topology inside runcard's analog.")
            coupler_targets = [
                f"c{couplers[0]}_{couplers[1]}" for couplers in self.platform.analog_compilation_settings.topology
            ]
            targets = [t for t in targets if t != "all_c"] + coupler_targets
        return targets
    ...
    def compile_nodes(self):
        ...
        idx = 1
        for node_type in self.calibration_config["node_types"]:  # TODO: remove node_type from calibration tree
            node_type_params: dict = self.calibration_config[node_type]
            for pipeline in node_type_params["pipelines"]:
                targets = self._check_topology(pipeline["targets"])
                ...
                for experiment in experiments:
                    ...
                    experiment_params: dict = CalibrationParser._fuse_dicts(
                        base_params, node_type_params.get(experiment, {})
                    )
                    if "calibration_path" in self.calibration_config:
                        experiment_params["calibration_path"] = self.calibration_config["calibration_path"]

                    if not any((base_params, experiment_params)):
                        logger.opt(colors=True).warning(
                            "No references found for <i><fg #8838ff>{experiment}</></i> or the base experiment in the calibration tree for targets <r>{targets}</r> (skipped).",
                            experiment=experiment,
                            targets=targets,
                        )
                        continue

                    simultaneous = experiment_params.get(
                        "simultaneous", base_params.get("simultaneous", pipeline_simultaneous)
                    )
