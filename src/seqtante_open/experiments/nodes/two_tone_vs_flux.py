...
def two_tone_frequency_vs_flux_node(platform: Platform, platform_path: str, parameters: dict[str, Any]):
    ...
    if not isinstance(crosstalk := calibration.crosstalk_matrix, CrosstalkMatrix):
        raise ValueError(
            "To execute two_tone_frequency_vs_flux experiment, the Calibration needs to have a CrosstalkMatrix"
        )
    ...
    def _run_experiment(
        ...
        drive_bus: str,
        readout_bus: str,
        readout_flux: tuple[str, float] | None = None,
    ):
    ...
    try:
        ...
        for coupler, loop in product(couplers, ["z", "x"][:coupler_loops][::-1]):
            ...
            _run_experiment(
                ...
                drive_bus=f"drive_{readout_qubit}",  # The coupler does not have fast lines so you can't drive directly to it so you need to drive trough the qubit.
                readout_bus=f"readout_{readout_qubit}",
                readout_flux=x_loop_readout_flux(readout_qubit, qubit_loops, parameters),
            )
