...
from seqtante_open.experiments.utils import get_operating_point
...
def t1_node(platform: Platform, platform_path: str, parameters: dict[str, Any]):
    ...
    if couplers:
        raise ValueError(f"t1 experiment does not execute on couplers. Found {couplers}")
    ...
    try:
        for qubit in qubits:
            ...
            model = T1Fit(measurement_id=measurement_id, target=qubit, path=target_params["data_folder"])
            model.fit()
            model.plot()
